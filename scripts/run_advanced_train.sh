...
LOG_FILE="${LOG_DIR}/${STAMP}_advanced_train.log"
LATEST_LOG="${LOG_DIR}/latest_advanced_train.log"
ln -sfn "${LOG_FILE}" "${LATEST_LOG}"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "[advanced_train] started_at=${STAMP}"
echo "[advanced_train] log=${LOG_FILE}"
